...
def _extract_oauth_refresh_error_reason(resp: httpx.Response) -> str:
    ...
    try:
        ...
        if isinstance(error_body, dict):
            ...
            raw_message = error_body.get("message") or error_body.get(
                "error_description"
            )
...
def _merge_refresh_failure_reason(
    current_reason: str | None, refresh_reason: str
) -> str | None:
...
def _store_refreshed_oauth_sync(
    ...
    with get_db_context() as db:
        ...
        ) is not None and not is_account_level_block(current_reason):
            key.oauth_invalid_at = None
    ...
    try:
        ...
        if provider_type == ProviderType.KIRO.value:
            ...
            (
                recheck_attempted,
                recheck_error,
            ) = await _refresh_account_state_after_oauth_update(
                provider_id=str(provider.id),
                provider_type=provider_type,
                key_ids=[key_id],
            )
...
async def _refresh_quota_after_import(
    ...
    if attempted and error:
        logger.warning(
            "[BATCH_IMPORT] 导入后配额刷新失败 (provider={}): {}", provider_id, error
        )
