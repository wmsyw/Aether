...
页面预览: https://wmsyw.github.io/Aether/
...
```bash
# 1. 克隆代码
git clone https://github.com/wmsyw/Aether.git
cd Aether
...
- GitHub OAuth 登录配置: [docs/oauth-github-setup.md](docs/oauth-github-setup.md)
- Passkey 通行密钥配置: [docs/passkey-setup.md](docs/passkey-setup.md)
...
#    将 docker-compose.yml 中 image 从 ghcr.io/wmsyw/aether:latest 改为指定版本
#    方式二：使用之前记录的镜像 digest
#    将 image 改为 ghcr.io/wmsyw/aether@sha256:xxxxx
docker compose up -d app
```

> 可以在升级前通过 `docker inspect ghcr.io/wmsyw/aether:latest --format '{{index .RepoDigests 0}}'` 记录当前镜像 digest，方便回滚时使用。
...
[![Star History Chart](https://api.star-history.com/svg?repos=wmsyw/Aether&type=Date)](https://star-history.com/#wmsyw/Aether&Date)
